...
typedef struct mapping
{
    ...
    void (*write_func)(uint16_t addr, uint8_t value);
} mapping_t;
...
uint8_t mapped_read(mapper_t *mapper, uint16_t addr);              // Read value from mapper
void mapped_write(mapper_t *mapper, uint16_t addr, uint8_t value); // Write value to mapper
